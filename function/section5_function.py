...
def line_scatter_plot(model_lines,evaluation,X,biomass,path=None):
    ...
    ax0.set_ylabel('地上干物质模拟值(kg/$m^2$)',fontsize=10,color='black')
    ax0.set_xlabel('地上干物质实测值(kg/$m^2$)',fontsize=10,color='black')
    ax0.legend(edgecolor='w',fontsize=9,loc=0)
    ax0.text(0.8,1.6,'y = {:.2f}x + {:.2f}\n$R^2$ = {:.2f}  '.format(model_best.coef_[0],model_best.intercept_,np.max(evaluation[:,0])))
...
def customer_interact_plot(customer_interact,customer_R_square,customer_names,path=None):
    ...
    colorlist = ['r','r','b','b','b','b','b','b','b','b','b','b','b','b','b','b','b','b','b']
...
def line_scatter_plot_2(model_best,evaluation_best,second_data_1,biomass,second_data_1_name,path=None):
    ...
    for k,i,j,name in zip(range(2),model_best,evaluation_best,second_data_1_name):
        ...
        ax.set_ylabel('地上干物质模拟值(kg/$m^2$)',fontsize=10,color='black')
        ax.set_xlabel('地上干物质实测值(kg/$m^2$)',fontsize=10,color='black')
        ax.legend(edgecolor='w',fontsize=9,loc=0)
        ax.text(0.9,1.6,'y = {:.2f}x + {:.2f}\n$R^2$ = {:.2f}  '.format(i.coef_[0],i.intercept_,j[0]),fontsize=13)
        ax.text(0.6,1.6,'{:s}'.format(name.strip('*')),fontsize=13)
        plt.tight_layout()
    if path:
        plt.savefig(path,dpi=200)
    plt.show()
...
def plot_doubelband(result,path=None):
    ...
    plt.title('a,差值植被指数与地上总干物质决定系数$R^2$',fontsize=20,color='black')
    ...
    plt.title('b,归一化植被指数与地上总干物质决定系数$R^2$',fontsize=20,color='black')
...
def line_scatter_plot_3(model_best_DVI,evaluation_best_DVI,DVI_pca,model_best_NDVI,evaluation_best_NDVI,NDVI_pca,lai,path=None):
    ...
    #ax0.set_yticks([4.6,5.0,5.4,5.8])
    #ax0.set_yticklabels([4.6,5.0,5.4,5.8],fontsize=10,color='black')
    ax0.set_ylabel('地上总干物质模拟值(kg/$m^2$)',fontsize=10,color='black')
    ax0.set_xlabel('地上总干物质实测值(kg/$m^2$)',fontsize=10,color='black')
    ax0.legend(edgecolor='w',fontsize=9,loc=0)
    ax0.text(0.9,1.6,'y = {:.2f}x + {:.2f}\n$R^2$ = {:.2f}  '.format(model_best_DVI.coef_[0],model_best_DVI.intercept_,evaluation_best_DVI[0]))
    ax0.text(0.6,1.6,'$a:DVI^1$$_{pca}$',fontsize=11)
    ...
    ax1.set_ylabel('地上总干物质模拟值(kg/$m^2$)',fontsize=10,color='black')
    ax1.set_xlabel('地上总干物质实测值(kg/$m^2$)',fontsize=10,color='black')
    ax1.legend(edgecolor='w',fontsize=9,loc=0)
    ax1.text(0.9,1.6,'y = {:.2f}x + {:.2f}\n$R^2$ = {:.2f}  '.format(model_best_NDVI.coef_[0],model_best_NDVI.intercept_,evaluation_best_NDVI[0]))
    ax1.text(0.6,1.6,'$b:NDVI^1$$_{pca}$',fontsize=11)
    plt.tight_layout()
    if path:
        plt.savefig(path,dpi=200)
    plt.show()
...
def save_models(models,name):
    ...
    for i,model in enumerate(models):
        filename  = 'models/biomass_' + name + '_' + str(i) + '.joblib'
...
def ridge_tree_svr_scatter(models,X,Y,path=None):
    ...
    for i,model in zip(range(1,4),models):
        ax = fig.add_subplot(1,3,i)
        if i == 1:
            ax.text(0.6,1.6,'a：岭回归',fontsize=11)
        if i == 2:
            ax.text(0.6,1.6,'b：决策树回归',fontsize=11)
        if i == 3:
            ax.text(0.4,1.6,'c：支持向量机回归（高斯核函数）',fontsize=11)
        ...
        ax.plot(line,line,color='black',label='1:1线')
        ax.set_ylabel('地上干物质重模拟值(kg/$m^2$)',fontsize=10,color='black')
        ax.set_xlabel('地上干物质重实测值(kg/$m^2$)',fontsize=10,color='black')
        ax.legend(edgecolor='w',fontsize=9,loc=0)
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
        ax.spines['left'].set_color('black')
...
def create_data10_60(pre_data,rate=[0.026,0.006,0.002,0.295,0.625,0.13]):
    #函数说明
    """
...
def plot_predict_10_60(pre_T1,pre_T2,pre_T3,models,path=None):
    ...
    for i,pre in zip(range(1,4),pre_T):
        ...
        scaler = StandardScaler()
        pre_data_2dim_scale = scaler.fit_transform(pre_data_2dim)
