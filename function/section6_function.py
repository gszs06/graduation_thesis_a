...
def evaluation_system(X,label):
    ...
    models = []
    number_70 = int(len(label)*0.7)
    for j in range(50):
        model = LinearRegression()
        random_index = [i for i in range(len(label))]
        np.random.shuffle(random_index)
        train_data = X[random_index[0:number_70]]
        train_label = label[random_index[0:number_70]]
        test_data = X[random_index[number_70:]]
        test_label = label[random_index[number_70:]]
...
def line_scatter_plot(model_lines,evaluation,X,biomass,path=None):
    #函数说明
    """
    ...
    biomass_predict = model_best.predict(X)
    plt.rcParams['font.sans-serif'] = ['SimHei']
    plt.rcParams['axes.unicode_minus'] = False
    fig = plt.figure(figsize=(5,4),dpi=200)
    ax0 = fig.add_subplot(111)
    ...
    ax0.scatter(biomass,biomass_predict,c='black',s=10)
    line = np.linspace(np.min([biomass,biomass_predict]),np.max([biomass,biomass_predict]),num=50,endpoint=True)
    ...
    ax0.set_ylabel('产量模拟值（kg/$m^2$）',fontsize=10,color='black')
    ax0.set_xlabel('产量实测值（kg/$m^2$）',fontsize=10,color='black')
    ax0.legend(edgecolor='w',fontsize=9,loc=0)
    ax0.text(0.65,0.85,'y = {:.2f}x + {:.2f}\n$R^2$ = {:.2f}  '.format(model_best.coef_[0],model_best.intercept_,np.max(evaluation[:,0])))
...
def customer_interact_plot(customer_interact,customer_R_square,customer_names,path=None):
    ...
    colorlist = ['r','b','b','b','b','b','b','b','b','b','b','b','b','b','b','b','b','b','b']
    ...
    ax2.set_xticklabels([customer_names[np.argsort(customer_R_square)[::-1]][0].strip('*'),
                    customer_names[np.argsort(customer_R_square)[::-1]][4].strip('*'),
...
def line_scatter_plot_2(model_best,evaluation_best,second_data_1,biomass,second_data_1_name,path=None):
    #函数说明
    """
    函数作用：绘制挑选出的2个已有的植被指数的线性模型的真实值与预测值的图像散点图及拟合方程
    input:
    ...
    plt.rcParams['axes.unicode_minus'] = False
    fig = plt.figure(figsize=(5,4),dpi=200)
    for k,i,j,name in zip(range(1),model_best,evaluation_best,second_data_1_name):
        ax = fig.add_subplot(1,1,k+1)
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
        ax.spines['left'].set_color('black')
        ax.spines['bottom'].set_color('black')
        biomass_predict = i.predict(second_data_1[:,k].reshape(-1,1))
        ax.scatter(biomass,biomass_predict,c='black',s=10)
        line = np.linspace(np.min([biomass,biomass_predict]),np.max([biomass,biomass_predict]),num=50,endpoint=True)
        ...
        ax.set_ylabel('产量模拟值（kg/$m^2$）',fontsize=10,color='black')
        ax.set_xlabel('产量实测值（kg/$m^2$）',fontsize=10,color='black')
        ax.legend(edgecolor='w',fontsize=9,loc=0)
        ax.text(0.65,0.82,'y = {:.2f}x + {:.2f}\n$R^2$ = {:.2f}  '.format(i.coef_[0],i.intercept_,j[0]),fontsize=13)
        ax.text(0.65,0.87,'{:s}'.format(name.strip('*')),fontsize=13)
        plt.tight_layout()
    if path:
        plt.savefig(path,dpi=200)
    plt.show()
...
def plot_doubelband(result,path=None):
    ...
    plt.title('a,差值植被指数与产量决定系数$R^2$',fontsize=20,color='black')
    ...
    plt.title('b,归一化植被指数与产量决定系数$R^2$',fontsize=20,color='black')
...
def line_scatter_plot_3(model_best_DVI,evaluation_best_DVI,DVI_pca,model_best_NDVI,evaluation_best_NDVI,NDVI_pca,lai,path=None):
    ...
    ax1.set_ylabel('产量模拟值（kg/$m^2$）',fontsize=10,color='black')
    ax1.set_xlabel('产量实测值（kg/$m^2$）',fontsize=10,color='black')
    ax1.legend(edgecolor='w',fontsize=9,loc=0)
    ax1.text(0.65,0.82,'y = {:.2f}x + {:.2f}\n$R^2$ = {:.2f}  '.format(model_best_NDVI.coef_[0],model_best_NDVI.intercept_,evaluation_best_NDVI[0]))
    ax1.text(0.65,0.87,'$b:NDVI^1$$_{pca}$',fontsize=11)
    plt.tight_layout()
    if path:
        plt.savefig(path,dpi=200)
    plt.show()
...
def save_models(models,name):
    ...
    for i,model in enumerate(models):
        filename  = 'models/output_28_' + name + '_' + str(i) + '.joblib'
...
def plot_Ridge_learning_curves(models,alphas,X,y,path=None):
    ...
    for model,i,alpha in zip(models,list(range(1,7)),alphas):
        ...
        ax.set_yticklabels([0,0.05,0.1,0.15,0.2],fontsize=14,color='black')
        plt.ylim(0,0.2)
        if i==1 or i==4:
            ax.set_ylabel('RMSE误差(kg/$m^2$)',fontsize=14,color='black')
...
"""模型应用"""
def ridge_tree_svr_scatter(models,X,Y,path=None):
    #函数说明
    """
    函数作用：绘制训练完成后岭回归、决策树回归和支持向量机回归的模拟值和实测值散点图
    input:
            models: 已经训练完成的三个模型
            X: 特征参数
            Y: 实际的地上干物质重
    ...
    plt.rcParams['axes.unicode_minus'] = False
    fig = plt.figure(figsize=(15,4),dpi=200)
    for i,model in zip(range(1,4),models):
        ax = fig.add_subplot(1,3,i)
        if i == 1:
            ax.text(0.65,0.85,'a：岭回归',fontsize=11)
        if i == 2:
            ax.text(0.65,0.85,'b：决策树回归',fontsize=11)
        if i == 3:
            ax.text(0.65,0.85,'c：支持向量机回归（高斯核函数）',fontsize=11)
            scaler = StandardScaler()
            X = scaler.fit_transform(X)
        pre_data = model.predict(X)
        ax.scatter(Y,pre_data,c='black',s=10)
        line = np.linspace(np.min([np.squeeze(Y),np.squeeze(pre_data)]),np.max([np.squeeze(Y),np.squeeze(pre_data)]),num=50,endpoint=True)
        ax.plot(line,line,color='black',label='1:1线')
        ax.set_ylabel('水稻产量模拟值(kg/$m^2$)',fontsize=10,color='black')
        ax.set_xlabel('水稻产量实测值(kg/$m^2$)',fontsize=10,color='black')
        ax.legend(edgecolor='w',fontsize=9,loc=0)
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
        ax.spines['left'].set_color('black')
...
def create_data10_60(pre_data,rate=[0.0041,0,0.0328,0.0461,0.4586]):
    #函数说明
    """
...
def plot_predict_10_60(pre_data,models,X,path=None):
    #函数说明
    """
    函数说明：绘制不同散射辐射比例下的水稻产量变化趋势
    input:
            pre_data: 散射辐射比例在10%下的各个参数值
            models: 训练完成的三个模型（岭回归、决策树回归和支持向量机回归）
            X: 训练模型的原始数据（支持向量机输入数据需要变换尺度，使用X来变换输入尺度）
            path: 保存绘图的路径
    output:
            无
    """
    ...
    plt.rcParams['axes.unicode_minus'] = False
    fig = plt.figure(figsize=(5,4),dpi=200)
    pre_data_2dim = create_data10_60(pre_data)
    pre_ridge = models[0].predict(pre_data_2dim)
    pre_tree = models[1].predict(pre_data_2dim)
    scaler = StandardScaler()
    scaler.fit_transform(X)
    pre_data_2dim = scaler.transform(pre_data_2dim)
    ...
    ax.legend(edgecolor='w',loc=1)
    ax.set_xticks([0,10,20,30,40,50])
    ax.set_xticklabels([10,20,30,40,50,60],color='black')
    ax.set_xlabel('散射辐射比例（%）',color='black')
    ax.set_ylabel('产量(kg/$m^2$)')
    plt.tight_layout()
    if path:
        plt.savefig(path)
    plt.show()        
